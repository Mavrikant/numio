...
describe("fuel-economy — compute", () => {
  it("converts 10 L/100km to 10 km/L", () => {
    ...
    const result = compute({ value: 10, fromUnit: "L_100km", toUnit: "km_L" });
    expect(result.result).toBeCloseTo(10, 1);
  });

  it("converts 25 mpg US to ~9.4 L/100km", () => {
    // 25 US miles per gallon ≈ 9.4 liters per 100km
    const result = compute({ value: 25, fromUnit: "mpg_us", toUnit: "L_100km" });
    expect(result.result).toBeCloseTo(9.404, 1);
  });

  it("converts 5 L/100km to 20 km/L", () => {
    // 5 L/100km (efficient) = 20 km/L
    const result = compute({ value: 5, fromUnit: "L_100km", toUnit: "km_L" });
    expect(result.result).toBeCloseTo(20, 1);
  });

  it("handles minimum positive input", () => {
    // Schema requires positive values (min 0.1), so test with small value
    const result = compute({ value: 0.1, fromUnit: "L_100km", toUnit: "km_L" });
