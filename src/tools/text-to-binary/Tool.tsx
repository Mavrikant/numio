...
export default function TextToBinaryTool({ locale }: { readonly locale: Locale }) {
  ...
  let error = "";
  try {
    output = text === "" ? "" : mode === "toBinary" ? textToBinary(text) : binaryToText(text);
  } catch {
    error = ui.invalid;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="inline-flex rounded-md border border-slate-300 p-0.5 dark:border-slate-600">
        {(["toBinary", "toText"] as const).map((m) => (
