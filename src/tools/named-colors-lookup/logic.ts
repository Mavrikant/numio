...
/** The 148 CSS named colors and their hex values. */
...
/** Filter named colors by a case-insensitive substring of the name or hex. */
export function searchColors(query: string): NamedColor[] {
  const q = query.trim().toLowerCase();
  if (!q) return NAMED_COLORS;
  return NAMED_COLORS.filter((c) => c.name.includes(q) || c.hex.includes(q));
}
